...
- 期号优先来自当前线路的本地时间窗口，而不是等待机器人边界消息；例如 PC28 某期 `start_time=17:55:00`、`next_time=17:58:30` 时，该窗口内的直接群下注归入该期。
- 有效统计窗口会排除 `start_time` 后前 10 秒，并在进入锁定阈值后停止统计；即统计区间为 `[start_time + 10s, next_time - lock_threshold_sec)`。
...
- 取消 = 删除该用户当前期全部下注记录，不按玩法或订单号做局部删除。
