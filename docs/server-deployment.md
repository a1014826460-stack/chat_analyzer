...
3. 将发布工具生成的 `latest.json` 和其中 `url` 指向的唯一安装包放入 `UPDATE_RELEASE_HOST_DIR`。容器内固定读取只读目录 `UPDATE_RELEASE_DIR=/srv/startrace/releases`；不得将客户端传入的 URL 用作服务端下载目标。
4. 调整 `docker-compose.yml` 的 API 端口映射：仅在 TLS 反向代理可用时对外暴露。IP 访问必须使用受信任 TLS 证书或客户端固定证书指纹后才允许上传 `user_sig` 和发送下注。
5. 执行 `sh backend/deploy/deploy.sh`，确认 `migrate` 已成功退出，`api` 的 readiness 为 `ok`，`worker` 处于运行状态。
6. 通过管理员 API 创建有限期、默认单设备的测试激活码；在客户端“帮助 -> 服务器模式”登录，上传测试 WSS 凭据，并验证待确认订单确认流程。
...
- 更新清单与安装包分别通过受 JWT 保护的 `/v1/updates/manifest` 和 `/v1/updates/files/{file_name}` 分发。服务端只允许下载当前清单引用的文件；桌面还会使用内置公钥验签清单并校验安装包 SHA-256。
