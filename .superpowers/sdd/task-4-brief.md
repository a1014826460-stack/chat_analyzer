...
**Goal:** Create the strategy engine that evaluates betting conditions and triggers injections.

**File to create:** `app/services/auto_bet_service.py`

**Dependencies:** Task 1 models (`app/models/auto_bet.py`) and Task 2 injector (`app/services/message_injector.py`) already committed.

## Requirements

1. `AutoBetService` class — strategy engine with background scheduling support
2. `apply_config(config: StrategyConfig)` — update strategy parameters
3. `set_injector(injector: MessageInjector | None)` — set the message injection backend
4. `set_result_provider(provider: DrawResultProvider | None)` — set the historical data source
5. `set_log_callback(callback)` — callback for GUI log updates
6. `start()` / `stop()` — lifecycle control
7. `tick(site, countdown_sec, current_period)` — called by GUI timer, evaluates and places bets
8. Trend-following: count consecutive same results from tail, if >= threshold → reverse bet
9. Dedup: track `_last_bet_period` to avoid double-betting same period
10. Lock threshold: skip betting when countdown <= lock_threshold_sec
11. Log cap: max 500 log entries
12. Thread safety: use `threading.Lock` for shared state
...
from app.models.auto_bet import StrategyConfig, DrawResult, BetDecision
...
cfg = StrategyConfig(site='pc28', bet_amount=50.0, observation_window=10, trigger_threshold=3)
svc.apply_config(cfg)
loaded = svc.config
assert loaded.site == 'pc28'
assert loaded.bet_amount == 50.0

# Test _opposite_play
assert AutoBetService._opposite_play('大', ['大', '小']) == '小'
assert AutoBetService._opposite_play('小', ['大', '小']) == '大'
assert AutoBetService._opposite_play('单', ['大', '小']) == '大'  # fallback
assert AutoBetService._opposite_play('大', ['大']) is None  # no opposite available

# Test tick with no provider (should not crash)
svc.start()
svc.tick('pc28', 60, '2026001')
svc.stop()

# Test log
logs = svc.get_logs()
assert len(logs) >= 2  # start + stop log entries
svc.clear_logs()
assert len(svc.get_logs()) == 0

print('All auto bet service tests passed!')
"
```
